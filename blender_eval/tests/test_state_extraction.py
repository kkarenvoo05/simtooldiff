...
try:
  ...
  _skip_reason = "CUDA not available or incompatible GPU"
  ...
  if _cuda_ok:
    try:
      ...
      _cuda_ok = False
      _skip_reason = "CUDA not available or incompatible GPU"
  if _cuda_ok and shutil.which("ninja") is None:
    _cuda_ok = False
    _skip_reason = "ninja required to JIT IsaacGym gymtorch extension"
except ImportError:
  _cuda_ok = False
  _skip_reason = "IsaacGym unavailable or imported after torch"

gpu = pytest.mark.skipif(not _cuda_ok, reason=_skip_reason)
