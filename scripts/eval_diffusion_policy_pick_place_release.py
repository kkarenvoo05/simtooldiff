...
<<<<<<< HEAD
    ...
    - Best-post-lift: the object was ever within xy tolerance AND moving slowly
      after the first lift. This catches "placed near goal then rolled off table"
      and "hand positioned over goal before dropping". Z is intentionally not
...
<<<<<<< HEAD
    ...
    # No z/on-table gate so this catches "hand over goal before releasing" and
    # "placed near goal then rolled off table" alike.
...
<<<<<<< HEAD
            ...
            # Raw metric values so failures can be diagnosed numerically.
            "max_object_height_above_init_m": round(float(max_object_height_above_init_m), 4),
            "final_transport_m": round(float(final_transport_m), 4),
            "final_place_xy_error_m": round(float(final_place_xy_error_m), 4),
            "final_place_z_error_m": round(float(final_place_z_error_m), 4),
            "final_object_speed_mps": round(float(final_object_speed_mps), 4),
            "dropped_after_lift": bool(dropped_after_lift),
            "reattempted_after_drop": bool(reattempted_after_drop),
            # Best post-lift placement: closest xy to goal (and speed at that
            # moment) seen at any step after the first lift. inf means the object
            # was never lifted above the min_lift_height threshold.
            "best_post_lift_xy_error_m": round(best_post_lift_xy_m, 4) if best_post_lift_xy_m != float("inf") else None,
            "best_post_lift_speed_mps": round(best_post_lift_speed_at_best_xy, 4) if best_post_lift_speed_at_best_xy != float("inf") else None,
            # Tolerances used for this episode (makes the JSON self-contained).
            "tol_xy_m": args.release_xy_tolerance,
            "tol_z_m": args.release_z_tolerance,
